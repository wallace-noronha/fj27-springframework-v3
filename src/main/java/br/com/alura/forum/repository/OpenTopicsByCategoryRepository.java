...
public interface OpenTopicsByCategoryRepository extends Repository<OpenTopicsByCategory, Long>{
	...
	@Query("select t from OpenTopicsByCategory t where year(t.date) = year(current_date) and month(t.date) = month(current_date)")
	List<OpenTopicsByCategoryRepository> findAllByCurrentMonth();
